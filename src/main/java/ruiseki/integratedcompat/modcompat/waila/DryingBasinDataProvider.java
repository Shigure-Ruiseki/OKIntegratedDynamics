...
public class DryingBasinDataProvider implements IWailaDataProvider {
    ...
    public List<String> getWailaBody(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,
        ...
        if (config.getConfig(Waila.getProxyConfigId())) {
            TileDryingBasin tile = TileHelpers
                .getSafeTile(accessor.getWorld(), new BlockPos(accessor.getPosition()), TileDryingBasin.class);
            if (tile != null) {
                if (tile.getStackInSlot(0) != null) {
                    currenttip.add(
                        LangHelpers.localize(
                            "gui." + Reference.MOD_ID + ".waila.item",
                ...
                if (!tile.getTank()
                    ...
                    currenttip.add(
                        LangHelpers.localize(
                            "gui." + Reference.MOD_ID + ".waila.fluid",
                            tile.getTank()
                                .getFluid()
                                .getLocalizedName(),
                            tile.getTank()
