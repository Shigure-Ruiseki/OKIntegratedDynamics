package ruiseki.integratedcompat.modcompat.jjfmuy.dryingbasin;
 ...
 * @author rubensworks
 */
public class DryingBasinRecipeCategory implements IRecipeCategory<DryingBasinRecipeWrapper> {

    public static final String UID = Reference.MOD_ID + ".drying_basin";

    public static void register(IRecipeCategoryRegistration registry) {
        IJFMUYHelpers jeiHelpers = registry.getJFMUYHelpers();
        IGuiHelper guiHelper = jeiHelpers.getGuiHelper();
        registry.addRecipeCategories(new DryingBasinRecipeCategory(guiHelper));
    }

    public static void initialize(IModRegistry registry) {
        try {
            registry.addRecipes(getRecipes(), UID);
            registry.addRecipeCatalyst(new ItemStack(BlockDryingBasinConfig._instance.getInstance()), UID);
        } catch (Throwable t) {
            IntegratedDynamics.clog(Level.ERROR, "Failed to initialize Drying Basin recipe category!", t);
        }
    }

    public static List<DryingBasinRecipeWrapper> getRecipes() {
        List<DryingBasinRecipeWrapper> wrappers = new ArrayList<>();
        List<RecipeDryingBasin> recipes = RecipeManager.getManager()
            .getAllRecipesFor(RecipeTypeDryingBasinConfig._instance.getInstance());

        for (RecipeDryingBasin recipe : recipes) {
            wrappers.add(new DryingBasinRecipeWrapper(recipe));
        }
        return wrappers;
    }
    ...
    public String getTitle() {
        return LangHelpers.localize(BlockDryingBasinConfig._instance.getFullUnlocalizedName());
    }
    ...
    @Override
    public void setRecipe(IRecipeLayout recipeLayout, DryingBasinRecipeWrapper recipe, IIngredients ingredients) {
