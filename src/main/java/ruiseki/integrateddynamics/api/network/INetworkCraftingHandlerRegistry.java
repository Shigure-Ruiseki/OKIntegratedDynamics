...
public interface INetworkCraftingHandlerRegistry extends IRegistry {
     ...
     */
    public default <T, M> boolean isCrafting(INetwork network,
        IPositionedAddonsNetworkIngredients<T, M> ingredientsNetwork, int channel,
        ...
        for (INetworkCraftingHandler craftingHandler : getCraftingHandlers()) {
            if (craftingHandler
                .isCrafting(network, ingredientsNetwork, channel, ingredientComponent, instance, matchCondition)) {
                return true;
            }
        }
        return false;
     ...
     */
    public default <T, M> boolean canCraft(INetwork network,
        IPositionedAddonsNetworkIngredients<T, M> ingredientsNetwork, int channel) {
        for (INetworkCraftingHandler craftingHandler : getCraftingHandlers()) {
     ...
     */
    public default <T, M> boolean craft(INetwork network, IPositionedAddonsNetworkIngredients<T, M> ingredientsNetwork,
        int channel, IngredientComponent<T, M> ingredientComponent, T instance, M matchCondition,
        boolean ignoreExistingJobs) {
        for (INetworkCraftingHandler craftingHandler : getCraftingHandlers()) {
            if (craftingHandler.craft(
                network,
                ingredientsNetwork,
                channel,
                ingredientComponent,
