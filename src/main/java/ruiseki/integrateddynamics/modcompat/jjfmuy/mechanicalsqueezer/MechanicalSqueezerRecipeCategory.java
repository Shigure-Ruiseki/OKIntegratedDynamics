package ruiseki.integrateddynamics.modcompat.jjfmuy.mechanicalsqueezer;
...
/**
 * Category for the Mechanical Drying Basin recipes.
 *
 * @author rubensworks
 */
public class MechanicalSqueezerRecipeCategory implements IRecipeCategory<MechanicalSqueezerRecipeWrapper> {

    public static final String UID = Reference.MOD_ID + ".mechanical_squeezer";

    public static void register(IRecipeCategoryRegistration registry) {
        IJFMUYHelpers jeiHelpers = registry.getJFMUYHelpers();
        IGuiHelper guiHelper = jeiHelpers.getGuiHelper();
        registry.addRecipeCategories(new MechanicalSqueezerRecipeCategory(guiHelper));
    }

    public static void initialize(IModRegistry registry) {
        try {
            registry.addRecipes(getRecipes(), UID);
            registry.addRecipeCatalyst(new ItemStack(BlockMechanicalSqueezerConfig._instance.getInstance()), UID);
        } catch (Throwable t) {
            IntegratedDynamics.clog(Level.ERROR, "Failed to initialize Squeezer recipe category!", t);
        }
    }

    public static List<MechanicalSqueezerRecipeWrapper> getRecipes() {
        List<MechanicalSqueezerRecipeWrapper> wrappers = new ArrayList<>();
        List<RecipeMechanicalSqueezer> recipes = RecipeManager.getManager()
            .getAllRecipesFor(RecipeTypeMechanicalSqueezerConfig._instance.getInstance());

        for (RecipeMechanicalSqueezer recipe : recipes) {
            wrappers.add(new MechanicalSqueezerRecipeWrapper(recipe));
        }
        return wrappers;
    }
    ...
    public String getTitle() {
        return LangHelpers.localize(BlockMechanicalSqueezerConfig._instance.getFullUnlocalizedName());
    }
    ...
    @Override
    public void setRecipe(IRecipeLayout recipeLayout, MechanicalSqueezerRecipeWrapper recipe,
