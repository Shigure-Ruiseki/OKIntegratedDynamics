...
public class NbtHelpers {
    ...
    private static void mergeInto(NBTTagCompound target, NBTTagCompound source) {
        for (String key : source.func_150296_c()) {
            ...
            if (sourceBase.getId() == 10 && target.hasKey(key, 10)) {
                mergeInto(target.getCompoundTag(key), (NBTTagCompound) sourceBase);
            } else {
                target.setTag(key, sourceBase.copy());
            }
        }
    }
    ...
    public static NBTTagCompound intersection(NBTTagCompound... tags) {
        ...
        for (NBTTagCompound inputTag : tags) {
            if (tag == null) {
                tag = (NBTTagCompound) inputTag.copy();
            } else {
                Set<String> keys = Sets.newHashSet(tag.func_150296_c());
                for (String key : keys) {
                    int type = tag.getTag(key)
                        .getId();
    ...
    public static NBTTagCompound minus(NBTTagCompound a, NBTTagCompound b) {
        NBTTagCompound tag = (NBTTagCompound) a.copy();
        for (String key : b.func_150296_c()) {
            int type = b.getTag(key)
                .getId();
