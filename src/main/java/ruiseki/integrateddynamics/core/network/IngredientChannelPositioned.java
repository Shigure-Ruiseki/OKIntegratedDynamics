...
public class IngredientChannelPositioned<T, M> extends IngredientChannelAdapter<T, M> {
    ...
    protected Iterator<PartPos> getAllPositions() {
        return getNetwork().getPositions(getChannel())
            .iterator();
    }
    ...
    public Iterator<T> iterator() {
        return new PositionedIngredientIterator<>(
            getNetwork(),
            getNetwork().getPositions(getChannel())
                .iterator(),
    ...
    public static class PositionedIngredientIterator<T, M> implements Iterator<T> {
        ...
        public PositionedIngredientIterator(IPositionedAddonsNetworkIngredients<T, M> network, Iterator<PartPos> it,
            T prototype, M matchFlags) {
        ...
        protected void prepareLastPos() {
            do {
                if (it.hasNext()) {
                    this.lastPos = this.network.getPositionedStorage(it.next())
