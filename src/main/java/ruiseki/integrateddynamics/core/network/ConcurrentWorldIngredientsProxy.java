...
public class ConcurrentWorldIngredientsProxy<T, M> {
    ...
    public void onWorldTick() {
        ...
        boolean positionsChanged = newPositions.size() != this.oldPositions.size()
            || !newPositions.containsAll(this.oldPositions);
        ...
        for (PartPos pos : newPositions) {
            ...
            if (this.readStates.contains(pos) || !this.states.containsKey(pos)) {
                IInventoryState inventoryState = CapabilityHelpers
                    .getCapability(pos.getPos(), InventoryStateConfig.CAPABILITY, pos.getSide())
                    .orElse(null);
