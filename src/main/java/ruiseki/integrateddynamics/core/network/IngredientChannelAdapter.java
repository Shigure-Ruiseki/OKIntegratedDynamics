...
public abstract class IngredientChannelAdapter<T, M> implements INetworkIngredientsChannel<T, M> {
    ...
    protected Pair<IPartPosIteratorHandler, Iterator<PartPos>> getPartPosIteratorData(
        Supplier<Iterator<PartPos>> iteratorSupplier, int channel) {
    ...
    public T insert(@Nonnull T ingredient, @Nonnull T unclaimed, boolean simulate) {
        ...
        ingredient = IIngredientChannelInsertPreConsumer
            .applyAll(network.getInsertPreConsumers(), matcher, this.channel, ingredient, unclaimed, simulate);
