...
public class NetworkDataClient {
    ...
    private static JsonElement getLocalizationJson() {
        ...
        for (String entry : LOCALIZE_ENTRIES) {
            jsonObject.addProperty(entry, LangHelpers.localize(entry));
        }
        return jsonObject;
    }
    ...
    private static PartPos parsePosString(String dimensionString, String positionString, String sideString) {
        ...
        Matcher matcher = INT_PATTERN.matcher(positionString);
        if (matcher.find()) x = Integer.parseInt(matcher.group());
        if (matcher.find()) y = Integer.parseInt(matcher.group());
        if (matcher.find()) z = Integer.parseInt(matcher.group());

        BlockPos pos = new BlockPos(x, y, z);

        int dimId = 0;
        try {
            dimId = Integer.parseInt(dimensionString.replaceAll("[^0-9-]", ""));
        } catch (NumberFormatException e) {
            IntegratedDynamics.clog("Invalid dimension ID: " + dimensionString);
        }

        ForgeDirection side = ForgeDirection.UNKNOWN;
        if (sideString != null && !sideString.equalsIgnoreCase("null")) {
            try {
                side = ForgeDirection.valueOf(sideString);
            } catch (IllegalArgumentException e) {
                side = ForgeDirection.UNKNOWN;
            }
        }

        return PartPos.of(DimPos.of(dimId, pos), side);
    }

    private static float getYawFromDirection(ForgeDirection side) {
        switch (side) {
            case NORTH:
                return 180F;
            case SOUTH:
    ...
    private static class ObservablePartData {
        ...
        public PartPos toPartPos() {
            ...
            if (world != null && world.provider.dimensionId == getDimension()) {
