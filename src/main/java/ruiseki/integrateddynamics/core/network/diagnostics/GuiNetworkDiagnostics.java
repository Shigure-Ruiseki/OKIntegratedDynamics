...
public class GuiNetworkDiagnostics extends JFrame {
                ...
                public void run() {
                    synchronized (networkDataParts) {
                        ...
                        for (ObservableObserverData observableObserverData : networkDataObservers.values()) {
                            ...
                            row.add(
                                ((double) observableObserverData.getLast20TicksDurationNs())
                                    / MinecraftHelpers.SECOND_IN_TICKS
                                    / 1000000);
                        ...
                        if (tableParts == null) {
                            ...
                            columnNamesParts
                                .addElement(LangHelpers.localize("gui.integrateddynamics.diagnostics.table.cables"));
                            columnNamesParts
                                .addElement(LangHelpers.localize("gui.integrateddynamics.diagnostics.table.part"));
                            columnNamesParts
                            ...
                            columnNamesParts
                                .addElement(LangHelpers.localize("gui.integrateddynamics.diagnostics.table.side"));
                            ...
                            tableObservers = new JTable(new DefaultTableModel(dataObservers, columnNamesObservers) {
                                ...
                                public Component prepareRenderer(TableCellRenderer renderer, int row, int column) {
                                    ...
                                    if (observerData != null && NetworkDiagnosticsPartOverlayRenderer.getInstance()
                                        .hasPartPos(observerData.toPartPos())) {
                                        c.setBackground(Color.CYAN);
                                    } else {
                                        c.setBackground(isCellSelected(row, column) ? getSelectionBackground() : null);
                                    }
                            ...
                            ((TableRowSorter<?>) tableParts.getRowSorter())
                                .setComparator(IDX_PARTS_POSITION, new BlockPosComparator());
                            ((TableRowSorter<?>) tableObservers.getRowSorter())
                                .setComparator(IDX_OBSERVERS_POSITION, new BlockPosComparator());
                            ...
                            panelParts.add(
                                BorderLayout.NORTH,
                                new JLabel(LangHelpers.localize("gui.integrateddynamics.diagnostics.parts")));
                            panelParts.add(BorderLayout.CENTER, new JScrollPane(tableParts));
                            panelObservers.add(
                                BorderLayout.NORTH,
                                new JLabel(LangHelpers.localize("gui.integrateddynamics.diagnostics.observers")));
    ...
    private static ObservablePartData getPartDataFromRow(int row) {
        ...
        Object[] data;
        synchronized (networkDataParts) {
            data = networkDataParts.values()
                .toArray();
        }
        int internalId = (int) tableParts.getModel()
            .getValueAt(tableParts.convertRowIndexToModel(row), IDX_PARTS_ID);
    ...
    private static ObservableObserverData getObserverDataFromRow(int row) {
        ...
        int internalId = (int) tableObservers.getModel()
            .getValueAt(tableObservers.convertRowIndexToModel(row), IDX_OBSERVERS_ID);
        if (internalId < data.length) {
    ...
    private static void teleportPlayer(MouseEvent e, PartPos pos) {
        ...
        if (e.getButton() == MouseEvent.BUTTON1) {
            ...
            BlockPos blockPos = pos.getPos()
                .getBlockPos()
                .offset(pos.getSide());
            ForgeDirection oppositeSide = pos.getSide()
                .getOpposite();
