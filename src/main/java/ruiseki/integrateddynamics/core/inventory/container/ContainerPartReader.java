...
public class ContainerPartReader<P extends IPartTypeReader<P, S> & IGuiContainerProvider, S extends IPartStateReader<P>>
    ...
    public void onDirty() {
        for (int i = 0; i < getUnfilteredItemCount(); i++) {
            ...
            if (itemStack != null && outputSlots.getStackInSlot(i) == null) {
                ItemStack outputStack = writeAspectInfo(
                    !getWorld().isRemote,
                    itemStack.copy(),
                    getUnfilteredItems().get(i));
    ...
    public void setReadValue(IAspectRead aspectRead, Pair<String, Integer> readValue) {
        int valueId = readValueIds.inverse()
            .get(aspectRead);
        int colorId = readColorIds.inverse()
            .get(aspectRead);
    ...
    public Pair<String, Integer> getReadValue(IAspectRead aspect) {
        ...
        try {
            return Pair.of(
                ValueNotifierHelpers.getValueString(this, valueId),
                ValueNotifierHelpers.getValueInt(this, colorId));
