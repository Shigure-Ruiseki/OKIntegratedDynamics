        ...
        if (isRenderTooltip()) {
            ...
            if (!container.hasWriteItemInSlot()) {
                if (gui.func_146978_c(
                    ...
                    List<String> tooltips = Lists
                        .newArrayList(LangHelpers.localize(L10NValues.GUI_LOGICPROGRAMMER_TOOLTIP_WRITESLOT_CREATE));
                    tooltips = Stream.concat(tooltips.stream(), getValueTypeTooltip(valueType).stream())
                        .toList();
                    gui.drawTooltip(tooltips, mouseX - guiLeft, mouseY - guiTop);
                }
            }
        }
    }
