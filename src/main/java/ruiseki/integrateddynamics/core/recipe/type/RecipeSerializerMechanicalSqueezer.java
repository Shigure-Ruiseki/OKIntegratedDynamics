...
public class RecipeSerializerMechanicalSqueezer implements IRecipeSerializer<RecipeMechanicalSqueezer> {
    ...
    protected static NonNullList<RecipeMechanicalSqueezer.ItemStackChance> getJsonItemStackChances(JsonObject json,
        String key) {
    ...
    public RecipeMechanicalSqueezer fromJson(ResourceLocation recipeId, JsonObject json) {
        ...
        // Output
        NonNullList<RecipeMechanicalSqueezer.ItemStackChance> outputItemStacks = getJsonItemStackChances(
            result,
            "items");
    ...
    @Nullable
    @Override
    public RecipeMechanicalSqueezer fromNetwork(ResourceLocation recipeId, ExtendedBuffer buffer) throws IOException {
        // Input
        Ingredient inputIngredient = Ingredient.fromNetwork(buffer);

        // Output Items
        NonNullList<RecipeMechanicalSqueezer.ItemStackChance> outputItemStacks = NonNullList.create();
        int outputItemStacksCount = buffer.readInt();
        for (int i = 0; i < outputItemStacksCount; i++) {
            outputItemStacks.add(
                new RecipeMechanicalSqueezer.ItemStackChance(buffer.readItemStackFromBuffer(), buffer.readFloat()));
        }
    ...
    @Override
    public void toNetwork(ExtendedBuffer buffer, RecipeMechanicalSqueezer recipe) throws IOException {
        // Input
        ...
        buffer.writeInt(
            recipe.getOutputItems()
                .size());
        for (RecipeMechanicalSqueezer.ItemStackChance outputItem : recipe.getOutputItems()) {
