...
public class ValueTypeLightLevelRegistry implements IValueTypeLightLevelRegistry {
    ...
    @Override
    public <L extends ILightLevelCalculator<V>, V extends IValue> L register(IValueType<V> valueType,
        L lightLevelCalculator) {
        ...
        for (Map.Entry<IValueType<?>, ILightLevelCalculator> entry : lightLevelCalculatorMap.entrySet()) {
            if (value.canCast(entry.getKey())) {
                return entry.getValue()
                    .getLightLevel(value.cast(entry.getKey()));
            }
        }
        throw new InvalidValueTypeException(
            String.format("The value %s can not be used to derive a light level.", value));
    }
