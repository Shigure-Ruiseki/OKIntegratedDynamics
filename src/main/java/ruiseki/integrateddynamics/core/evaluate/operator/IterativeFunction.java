...
public class IterativeFunction implements OperatorBase.IFunction {
    ...
    public static class PrePostBuilder<O, P> {
        ...
        private PrePostBuilder(List<IOperatorValuePropagator<?, ?>> preValuePropagators,
            List<IOperatorValuePropagator<?, ?>> postValuePropagators) {
        ...
        public static PrePostBuilder<OperatorBase.SafeVariablesGetter, IValue> begin() {
            return new PrePostBuilder<>(
                Collections.<IOperatorValuePropagator<?, ?>>emptyList(),
                Collections.<IOperatorValuePropagator<?, ?>>emptyList());
        }
        ...
        public <O2> PrePostBuilder<O2, P> appendPre(IOperatorValuePropagator<O, O2> valuePropagator) {
            return new PrePostBuilder<>(
                Helpers.joinList(preValuePropagators, valuePropagator),
                Lists.newArrayList(postValuePropagators));
        }
        ...
        public <P2> PrePostBuilder<O, P2> appendPost(IOperatorValuePropagator<P2, P> valuePropagator) {
            return new PrePostBuilder<>(
                Lists.newArrayList(preValuePropagators),
                Helpers.joinList(postValuePropagators, valuePropagator));
        }
        ...
        public IterativeFunction build(IOperatorValuePropagator<O, P> valuePropagator) {
            List<IOperatorValuePropagator<?, ?>> valuePropagators = Lists
                .newArrayListWithExpectedSize(preValuePropagators.size() + postValuePropagators.size());
