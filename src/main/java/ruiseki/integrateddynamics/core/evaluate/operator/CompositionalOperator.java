...
public class CompositionalOperator extends OperatorBase {
    ...
    public static class AppliedOperatorBuilder {
        ...
        public AppliedOperatorBuilder apply(AppliedOperatorBuilder... builders) {
            ...
            LangHelpers.UnlocalizedString error = this.base.validateTypes(valueTypes);
            if (error != null) {
            ...
            if (buildersRequiredInputLength == -1) {
                if (this.builders != null) {
                    ...
                    for (AppliedOperatorBuilder builder : builders) {
                        this.buildersRequiredInputLength = Math
                            .min(this.buildersRequiredInputLength, builder.getRequiredInputLength());
                    }
                } else {
                            ...
                            throw new IllegalArgumentException(
                                String.format(
                                    "An composed operator expected type %s "
                                        + "at position %s, while a type of %s was found.",
                                    inputTypes[i],
                                    i,
                                    subInputTypes[i]));
        ...
        protected LangHelpers.UnlocalizedString validateTypes(String unlocalizedOperatorName, IValueType[] input) {
            if (this.builders == null) {
                ...
                if (input.length != getRequiredInputLength()) {
                    return new LangHelpers.UnlocalizedString(
                        ...
                        new LangHelpers.UnlocalizedString(unlocalizedOperatorName),
                        input.length,
                ...
                for (int i = 0; i < builders.length; i++) {
                    ...
                    LangHelpers.UnlocalizedString subError;
         ...
         */
        public IOperator build(String symbol, String operatorName, IConfigRenderPattern renderPattern,
            String unlocalizedType) {
            return new CompositionalOperator(
                symbol,
                operatorName,
                getInputTypes(),
                this.base.getOutputType(),
                new IFunction() {
