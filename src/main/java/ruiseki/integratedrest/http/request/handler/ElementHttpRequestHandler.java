            ...
            if (tile != null) {
                if (tile.getProxyId() == id) {
                    if (request.getMethod()
                        ...
                        .equals(HttpMethod.POST) && request instanceof HttpContent) {
                            String content = ((HttpContent) request).content()
                            ...
                            try {
                                ...
                                if (jsonElement == null) {
                                    responseObject
                                        .addProperty("error", "No value property was found in the root object.");
                                    return HttpResponseStatus.BAD_REQUEST;
                                }
                                Optional<IValue> valueOptional = JsonUtil.jsonToValue(jsonElement);
                                if (valueOptional.isPresent()) {
                                    IValue value = valueOptional.get();
                                    if (tile.getValueType()
                                        .correspondsTo(value.getType())) {
                                        tile.setValue(valueOptional.get());
                                    } else {
                                        responseObject.addProperty(
                                            "error",
                                            "Invalid value type, HTTP Proxy expects " + tile.getValueType()
                                                .getTypeName()
                                                + " but "
                                                + value.getType()
                                                    .getTypeName()
