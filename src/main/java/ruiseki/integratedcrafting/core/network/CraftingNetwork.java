package ruiseki.integratedcrafting.core.network;
...
import ruiseki.integratedcrafting.api.recipe.IRecipeIndexModifiable;
import ruiseki.integratedcrafting.core.CraftingHelpers;
import ruiseki.integratedcrafting.core.CraftingJobIndexDefault;
import ruiseki.integratedcrafting.core.RecipeIndexDefault;
...
public class CraftingNetwork implements ICraftingNetwork {
    ...
    protected static Multimap<IRecipeDefinition, ICraftingInterface> newRecipeCraftingInterfacesMap() {
        return MultimapBuilder.hashKeys()
            .treeSetValues(ICraftingInterface.createComparator())
            .build();
    }

    @Override
    public int[] getChannels() {
        return craftingInterfaces.keySet()
            .toIntArray();
    }
    ...
    public Multimap<IRecipeDefinition, ICraftingInterface> getRecipeCraftingInterfaces(int channel) {
        ...
        Multimap<IRecipeDefinition, ICraftingInterface> recipeCraftingInterfaces = this.recipeCraftingInterfaces
            .get(channel);
    ...
    public boolean addCraftingInterface(int channel, ICraftingInterface craftingInterface) {
        ...
        if (getCraftingInterfaces(channel).add(craftingInterface)) {
            ...
            Multimap<IRecipeDefinition, ICraftingInterface> recipeCraftingInterfaces = getRecipeCraftingInterfaces(
                channel);
            ...
            while (craftingJobsIt.hasNext()) {
                ...
                IntListIterator dependencyIt = craftingJob.getDependencyCraftingJobs()
                    .iterator();
    ...
    @Override
    public boolean addCraftingInterfaceRecipe(int channel, ICraftingInterface craftingInterface,
    ...
    @Override
    public boolean removeCraftingInterfaceRecipe(int channel, ICraftingInterface craftingInterface,
        IRecipeDefinition recipe) {
        IRecipeIndexModifiable recipeIndex = getRecipeIndex(channel);
        Multimap<IRecipeDefinition, ICraftingInterface> recipeCraftingInterfaces = getRecipeCraftingInterfaces(channel);
    ...
    public void scheduleCraftingJob(CraftingJob craftingJob, boolean allowDistribution,
        ...
        if (allowDistribution && craftingInterfaces.size() > 1 && craftingJob.getAmount() > 1) {
            Collection<CraftingJob> splitCraftingJobs = CraftingHelpers.splitCraftingJobs(
                craftingJob,
                craftingInterfaces.size(),
    ...
    protected long getCurrentTick() {
        return FMLCommonHandler.instance()
            .getMinecraftServerInstance().worldServers[0].getTotalWorldTime();
    }
    ...
    public Iterator<CraftingJob> getCraftingJobs(int channel) {
        return new MultitransformIterator<>(
            getCraftingInterfaces(channel).iterator(),
    ...
    public <T, M> Iterator<CraftingJob> getCraftingJobs(int channel, IngredientComponent<T, M> ingredientComponent,
        ...
        // Check for the case the crafting job was explicitly started on the wildcard channel
        ICraftingJobIndexModifiable wildcardCraftingJobIndex = indexedCraftingJobs
        ...
        if (wildcardCraftingJobIndex != null) {
            wildcardChannelIterator = wildcardCraftingJobIndex
                .getCraftingJobs(ingredientComponent, instance, matchCondition);
        } else {
