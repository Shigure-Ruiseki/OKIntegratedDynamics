...
import ruiseki.integratedcrafting.api.crafting.FailedCraftingRecipeException;
import ruiseki.integratedcrafting.api.crafting.ICraftingResultsSink;
import ruiseki.integratedcrafting.api.crafting.RecursiveCraftingRecipeException;
import ruiseki.integratedcrafting.api.crafting.UnavailableCraftingInterfacesException;
...
import ruiseki.okcore.ingredient.collection.IngredientCollectionQuantitativeGrouper;
import ruiseki.okcore.ingredient.collection.IngredientHashSet;
...
public class CraftingHelpers {
    ...
    public static <T, M> CraftingJob calculateCraftingJobs(INetwork network, int channel,
        IngredientComponent<T, M> ingredientComponent, T instance, M matchCondition, boolean craftMissing,
        IIdentifierGenerator identifierGenerator, CraftingJobDependencyGraph craftingJobsGraph,
        boolean collectMissingRecipes) throws UnknownCraftingRecipeException, RecursiveCraftingRecipeException {
        ...
        Function<IngredientComponent<?, ?>, IIngredientComponentStorage> storageGetter = getNetworkStorageGetter(
            network,
            channel,
            true);
     ...
     * @throws RecursiveCraftingRecipeException If an infinite recursive recipe was detected.
     */
    public static CraftingJob calculateCraftingJobs(INetwork network, int channel, IRecipeDefinition recipe, int amount,
        boolean craftMissing, IIdentifierGenerator identifierGenerator, CraftingJobDependencyGraph craftingJobsGraph,
        boolean collectMissingRecipes) throws FailedCraftingRecipeException, RecursiveCraftingRecipeException {
        ICraftingNetwork craftingNetwork = getCraftingNetworkChecked(network);
        IRecipeIndex recipeIndex = craftingNetwork.getRecipeIndex(channel);
        Function<IngredientComponent<?, ?>, IIngredientComponentStorage> storageGetter = getNetworkStorageGetter(
            network,
        ...
        if (result.getCraftingJob() == null) {
            throw new FailedCraftingRecipeException(
                recipe,
                amount,
                result.getMissingDependencies(),
                compressMixedIngredients(new MixedIngredients(result.getIngredientsStorage())),
                result.getPartialCraftingJobs());
        } else {
    ...
    protected static <T, M> CraftingJob calculateCraftingJobs(IRecipeIndex recipeIndex, int channel,
        ...
        IngredientComponent<T, M> ingredientComponent, T instance, M matchCondition, boolean craftMissing,
        Map<IngredientComponent<?, ?>, IngredientCollectionPrototypeMap<?, ?>> simulatedExtractionMemory,
        Map<IngredientComponent<?, ?>, IIngredientCollectionMutable<?, ?>> extractionMemoryReusable,
        IIdentifierGenerator identifierGenerator, CraftingJobDependencyGraph craftingJobsGraph,
        Set<IPrototypedIngredient> parentDependencies, boolean collectMissingRecipes)
        throws UnknownCraftingRecipeException, RecursiveCraftingRecipeException {
        IIngredientMatcher<T, M> matcher = ingredientComponent.getMatcher();
        // This matching condition makes it so that the recipe output does not have to match with the requested input by
        // quantity.
        M quantifierlessCondition = matcher.withoutCondition(
            matchCondition,
        ...
        Iterator<IRecipeDefinition> recipes = recipeIndex
            .getRecipes(ingredientComponent, instance, quantifierlessCondition);
        ...
        while (recipes.hasNext()) {
            ...
            long recipeOutputQuantity = getOutputQuantityForRecipe(
                recipe,
                ingredientComponent,
                instance,
                quantifierlessCondition);
        ...
        throw new UnknownCraftingRecipeException(
            new PrototypedIngredient<>(ingredientComponent, instance, matchCondition),
            matcher.getQuantity(instance),
            firstMissingDependencies,
            compressMixedIngredients(new MixedIngredients(firstIngredientsStorage)),
     ...
     * @throws RecursiveCraftingRecipeException If an infinite recursive recipe was detected.
     */
    protected static PartialCraftingJobCalculation calculateCraftingJobs(IRecipeIndex recipeIndex, int channel,
        Function<IngredientComponent<?, ?>, IIngredientComponentStorage> storageGetter, IRecipeDefinition recipe,
        int amount, boolean craftMissing,
        Map<IngredientComponent<?, ?>, IngredientCollectionPrototypeMap<?, ?>> simulatedExtractionMemory,
        Map<IngredientComponent<?, ?>, IIngredientCollectionMutable<?, ?>> extractionMemoryReusable,
        IIdentifierGenerator identifierGenerator, CraftingJobDependencyGraph craftingJobsGraph,
        Set<IPrototypedIngredient> parentDependencies, boolean collectMissingRecipes)
        ...
        Pair<Map<IngredientComponent<?, ?>, List<?>>, Map<IngredientComponent<?, ?>, MissingIngredients<?, ?>>> simulation = getRecipeInputs(
            storageGetter,
            recipe,
            true,
            simulatedExtractionMemory,
        ...
        if (!craftMissing && !missingIngredients.isEmpty()) {
            if (collectMissingRecipes) {
                ...
                for (Map.Entry<IngredientComponent<?, ?>, MissingIngredients<?, ?>> entry : missingIngredients
                    .entrySet()) {
                    for (MissingIngredients.Element<?, ?> element : entry.getValue()
                        .getElements()) {
                        MissingIngredients.PrototypedWithRequested<?, ?> alternative = element.getAlternatives()
                            .get(0);

                        // Calculate the instance that was available in storage
                        IngredientComponent<?, ?> component = alternative.getRequestedPrototype()
                            .getComponent();
                        IIngredientMatcher matcher = component.getMatcher();
                        long storedQuantity = matcher.getQuantity(
                        ...
                        missingDependencies.add(
                            new UnknownCraftingRecipeException(
                                alternative.getRequestedPrototype(),
                                alternative.getQuantityMissing(),
                                Collections.emptyList(),
                                compressMixedIngredients(new MixedIngredients(storageMap)),
                                Lists.newArrayList()));
                    }
                }
            }
        ...
        Map<IngredientComponent<?, ?>, IngredientCollectionPrototypeMap<?, ?>> dependenciesOutputSurplus = Maps
            .newIdentityHashMap();
        ...
        for (IngredientComponent dependencyComponent : missingIngredients.keySet()) {
            try {
                // TODO: if we run into weird simulated extraction bugs, we may have to scope simulatedExtractionMemory,
        ...
        for (IngredientComponent<?, ?> surplusComponent : dependenciesOutputSurplus.keySet()) {
            ...
            if (surplusInstances != null) {
                for (Object instance : surplusInstances) {
                    IngredientCollectionPrototypeMap<?, ?> simulatedExtractionMemoryInstances = simulatedExtractionMemory
                        .get(surplusComponent);
                    if (simulatedExtractionMemoryInstances == null) {
                        simulatedExtractionMemoryInstances = new IngredientCollectionPrototypeMap<>(
                            surplusComponent,
                            true);
        ...
        if (!missingDependencies.isEmpty()) {
            return new PartialCraftingJobCalculation(
                null,
                missingDependencies,
                simulation.getLeft(),
                partialCraftingJobs);
    ...
    protected static <T, M> PartialCraftingJobCalculationDependency calculateCraftingJobDependencyComponent(
        ...
        MissingIngredients<T, M> missingIngredients, Set<IPrototypedIngredient> parentDependencies,
        Map<IRecipeDefinition, CraftingJob> dependencies, IRecipeIndex recipeIndex, int channel,
        Function<IngredientComponent<?, ?>, IIngredientComponentStorage> storageGetter,
        Map<IngredientComponent<?, ?>, IngredientCollectionPrototypeMap<?, ?>> simulatedExtractionMemory,
        Map<IngredientComponent<?, ?>, IIngredientCollectionMutable<?, ?>> extractionMemoryReusable,
        IIdentifierGenerator identifierGenerator, CraftingJobDependencyGraph craftingJobsGraph,
        boolean collectMissingRecipes) throws RecursiveCraftingRecipeException {
        ...
        for (MissingIngredients.Element<T, M> missingElement : missingIngredients.getElements()) {
            ...
            for (MissingIngredients.PrototypedWithRequested<T, M> prototypedAlternative : missingElement
                .getAlternatives()) {
                // Check if the missing element is reusable, and was triggered for craft earlier.
                if (missingElement.isInputReusable()
                    && ((IIngredientCollectionMutable<T, M>) extractionMemoryReusable.get(dependencyComponent))
                        .contains(
                            prototypedAlternative.getRequestedPrototype()
                                .getPrototype())) {
                ...
                IPrototypedIngredient<T, M> prototype = new PrototypedIngredient<>(
                    dependencyComponent,
                    dependencyMatcher.withQuantity(
                        prototypedAlternative.getRequestedPrototype()
                            .getPrototype(),
                        prototypedAlternative.getQuantityMissing()),
                    prototypedAlternative.getRequestedPrototype()
                        .getCondition());
                // First check if we can grab it from previous surplus
                IngredientCollectionPrototypeMap<T, M> dependencyComponentSurplusOld = (IngredientCollectionPrototypeMap<T, M>) dependenciesOutputSurplus
                ...
                if (dependencyComponentSurplusOld != null) {
                    ...
                    dependencyComponentSurplus = new IngredientCollectionPrototypeMap<>(
                        dependencyComponentSurplusOld.getComponent(),
                        true);
                    ...
                    IIngredientMatcher<T, M> prototypeMatcher = prototype.getComponent()
                        .getMatcher();
                    ...
                    Iterator<T> surplusIt = dependencyComponentSurplus.iterator(
                        prototype.getPrototype(),
                        prototypeMatcher.withoutCondition(
                    ...
                    if (updatedRemainingQuantity) {
                        if (remainingQuantity == 0) {
                            ...
                            prototype = new PrototypedIngredient<>(
                                dependencyComponent,
                ...
                try {
                    ...
                    if (dependencyMatcher.getQuantity(dependencyPrototype.getPrototype()) != 1) {
                        ...
                        dependencyPrototype = new PrototypedIngredient<>(
                            dependencyComponent,
                    ...
                    dependency = calculateCraftingJobs(
                        recipeIndex,
                        channel,
                        storageGetter,
                        dependencyComponent,
                    ...
                    Object dependencyQuantifierlessCondition = dependencyMatcher.withoutCondition(
                        prototype.getCondition(),
                        dependencyComponent.getPrimaryQuantifier()
                            .getMatchCondition());
                    long requestedQuantity = dependencyMatcher.getQuantity(prototype.getPrototype());
                    for (IngredientComponent outputComponent : dependency.getRecipe()
                        .getOutput()
                        .getComponents()) {
                        IngredientCollectionPrototypeMap<?, ?> componentSurplus = dependenciesOutputSurplus
                            .get(outputComponent);
                        ...
                        List<Object> instances = dependency.getRecipe()
                            .getOutput()
                            .getInstances(outputComponent);
                        long recipeAmount = dependency.getAmount();
                        if (recipeAmount > 1) {
                            IIngredientMatcher matcher = outputComponent.getMatcher();
                            // If more than one recipe amount was crafted, correctly multiply the outputs to calculate
                            // the effective surplus.
                            instances = instances.stream()
                                .map(
                                    instance -> matcher
                                        .withQuantity(instance, matcher.getQuantity(instance) * recipeAmount))
                                .collect(Collectors.toList());
                        }
                        addRemainderAsSurplusForComponent(
                            outputComponent,
                            (List) instances,
                            componentSurplus,
                            (IngredientComponent) prototype.getComponent(),
                            prototype.getPrototype(),
                            dependencyQuantifierlessCondition,
                            requestedQuantity);
                    }
                    ...
                    if (firstError == null) {
                        ...
                        firstError = new UnknownCraftingRecipeException(
                            e.getIngredient(),
                            prototypedAlternative.getQuantityMissing(),
                            e.getMissingChildRecipes(),
                            compressMixedIngredients(e.getIngredientsStorage()),
                            e.getPartialCraftingJobs());
                    }
                }
            }
    ...
    protected static <T1, M1, T2, M2> void addRemainderAsSurplusForComponent(
        IngredientComponent<T1, M1> ingredientComponent, List<T1> instances,
        IngredientCollectionPrototypeMap<T1, M1> simulatedExtractionMemory,
        IngredientComponent<T2, M2> blackListComponent, T2 blacklistInstance, M2 blacklistCondition,
        ...
        for (T1 instance : instances) {
            ...
            long quantity = simulatedExtractionMemory.getQuantity(instance)
                + (outputMatcher.getQuantity(instance) - reduceQuantity);
            if (quantity > 0) {
    ...
    public static void scheduleCraftingJobs(ICraftingNetwork craftingNetwork,
        Function<IngredientComponent<?, ?>, IIngredientComponentStorage> storageGetter,
        CraftingJobDependencyGraph craftingJobDependencyGraph, boolean allowDistribution, @Nullable UUID initiator)
        throws UnavailableCraftingInterfacesException {
        List<CraftingJob> startedJobs = Lists.newArrayList();
        craftingNetwork.getCraftingJobDependencyGraph()
            .importDependencies(craftingJobDependencyGraph);
        for (CraftingJob craftingJob : craftingJobDependencyGraph.getCraftingJobs()) {
            try {
                ...
                for (CraftingJob startedJob : startedJobs) {
                    CraftingHelpers.insertIngredientsGuaranteed(
                        startedJob.getIngredientsStorageBuffer(),
                        storageGetter,
                        (ICraftingResultsSink) Iterables
                            .getFirst(craftingNetwork.getCraftingInterfaces(startedJob.getChannel()), null));
    ...
    public static CraftingJob scheduleCraftingJob(ICraftingNetwork craftingNetwork,
        Function<IngredientComponent<?, ?>, IIngredientComponentStorage> storageGetter, CraftingJob craftingJob,
        boolean allowDistribution, @Nullable UUID initiator) throws UnavailableCraftingInterfacesException {
        craftingNetwork.scheduleCraftingJob(craftingJob, allowDistribution, storageGetter);
        if (initiator != null) {
            craftingJob.setInitiatorUuid(initiator.toString());
        }
    ...
    public static <T, M> CraftingJob calculateAndScheduleCraftingJob(INetwork network, int channel,
        IngredientComponent<T, M> ingredientComponent, T instance, M matchCondition, boolean craftMissing,
        boolean allowDistribution, IIdentifierGenerator identifierGenerator, @Nullable UUID initiator) {
        try {
            ...
            scheduleCraftingJobs(
                craftingNetwork,
                getNetworkStorageGetter(network, channel, false),
                dependencyGraph,
                allowDistribution,
                initiator);

            return craftingJob;
        } catch (UnknownCraftingRecipeException | RecursiveCraftingRecipeException
            | UnavailableCraftingInterfacesException e) {
            return null;
        }
    }
     ...
     */
    @Nullable
    public static CraftingJob calculateAndScheduleCraftingJob(INetwork network, int channel, IRecipeDefinition recipe,
        int amount, boolean craftMissing, boolean allowDistribution, IIdentifierGenerator identifierGenerator,
        ...
        try {
            ...
            return craftingJob;
        } catch (RecursiveCraftingRecipeException | FailedCraftingRecipeException
            | UnavailableCraftingInterfacesException e) {
            return null;
        }
    }
    ...
    public static <T, M> boolean hasStorageInstance(INetwork network, int channel,
        ...
        if (storage instanceof IngredientChannelIndexed) {
            IIngredientMatcher<T, M> matcher = ingredientComponent.getMatcher();
            long quantityPresent = ((IngredientChannelIndexed<T, M>) storage).getIndex()
                .getQuantity(instance);
            contains = matcher.hasCondition(
                matchCondition,
                ingredientComponent.getPrimaryQuantifier()
                    .getMatchCondition()) ? quantityPresent >= matcher.getQuantity(instance) : quantityPresent > 0;
        } else {
    ...
    public static <T, M> boolean isCrafting(ICraftingNetwork craftingNetwork, int channel,
        IngredientComponent<T, M> ingredientComponent, T instance, M matchCondition) {
        Iterator<CraftingJob> craftingJobs = craftingNetwork
     ...
     * @param simulate             If true, then the ingredients will effectively be removed from the network, not when
     *                             false.
    ...
    public static <T, M> List<T> getIngredientRecipeInputs(IIngredientComponentStorage<T, M> storage,
        IngredientComponent<T, M> ingredientComponent, IRecipeDefinition recipe, boolean simulate,
        long recipeOutputQuantity) {
        return getIngredientRecipeInputs(
            storage,
            ingredientComponent,
            recipe,
            simulate,
     ...
     * @param ingredientComponent       The ingredient component to get the ingredients for.
     * @param recipe                    The recipe to get the inputs from.
     * @param simulate                  If true, then the ingredients will effectively be removed from the network, not
     *                                  when false.
     * @param simulatedExtractionMemory This map remembers all extracted instances in simulation mode.
     *                                  This is to make sure that instances can not be extracted multiple times
     *                                  when simulating.
    ...
    public static <T, M> Pair<List<T>, MissingIngredients<T, M>> getIngredientRecipeInputs(
        ...
        IIngredientCollectionMutable<T, M> extractionMemoryReusable, boolean collectMissingIngredients,
        long recipeOutputQuantity) {
        return getIngredientRecipeInputs(
            storage,
            ingredientComponent,
    ...
    public static <T, M> Pair<List<T>, MissingIngredients<T, M>> getIngredientRecipeInputs(
        ...
        IRecipeDefinition recipe, boolean simulate, IngredientCollectionPrototypeMap<T, M> simulatedExtractionMemory,
        IIngredientCollectionMutable<T, M> extractionMemoryReusable, boolean collectMissingIngredients,
        long recipeOutputQuantity, boolean skipReusableIngredients) {
        IIngredientMatcher<T, M> matcher = ingredientComponent.getMatcher();
            ...
            if (collectMissingIngredients) {
                ...
                MissingIngredients<T, M> missing = new MissingIngredients<>(
                    recipeInputs.stream()
                        ...
                        .map(l -> multiplyPrototypedIngredients(l, recipeOutputQuantity)) // If the input is reusable,
                                                                                          // don't multiply the expected
                                                                                          // input quantity
                        .map(
                            ps -> new MissingIngredients.Element<>(
                                ps.stream()
                                    .map(
                                        p -> new MissingIngredients.PrototypedWithRequested<>(
                                            p,
                                            matcher.getQuantity(p.getPrototype())))
                                    .collect(Collectors.toList()),
                                false))
                        .collect(Collectors.toList()));
                return Pair.of(
        ...
        List<IPrototypedIngredientAlternatives<T, M>> inputAlternativePrototypes = recipe
            .getInputs(ingredientComponent);
        ...
        for (int inputIndex = 0; inputIndex < inputAlternativePrototypes.size(); inputIndex++) {
            ...
            IngredientCollectionPrototypeMap<T, M> simulatedExtractionMemoryAlternative = simulate
                ? new IngredientCollectionPrototypeMap<>(ingredientComponent, true)
                : null;
            if (simulate) {
            ...
            for (IPrototypedIngredient<T, M> inputPrototype : inputPrototypes.getAlternatives()) {
                ...
                IngredientCollectionPrototypeMap<T, M> simulatedExtractionMemoryBuffer = simulate
                    ? new IngredientCollectionPrototypeMap<>(ingredientComponent, true)
                    : null;
                IIngredientCollectionMutable<T, M> extractionMemoryReusableBuffer = inputReusable
                    ? new IngredientHashSet<>(ingredientComponent)
                    : null;
                ...
                if (inputReusable && extractionMemoryReusable.contains(inputPrototype.getPrototype())) {
                    ...
                    inputInstance = inputPrototype.getComponent()
                        .getMatcher()
                        .getEmptyInstance();
                    hasInputInstance = true;
                    shouldBreak = true;
                    ...
                    if (simulate && (memoryQuantity = simulatedExtractionMemoryAlternative
                        ...
                        if (newQuantity > 0) {
                            ...
                            M matchCondition = matcher.withoutCondition(
                                inputPrototype.getCondition(),
                                ingredientComponent.getPrimaryQuantifier()
                                    .getMatchCondition());
                            ...
                            if (quantityExtracted == newQuantity) {
                                ...
                                missingAlternatives.add(
                                    new MissingIngredients.PrototypedWithRequested<>(
                                        inputPrototype,
                                        quantityMissingRelative));
                                inputInstance = matcher.withQuantity(
                                    inputPrototype.getPrototype(),
                                    prototypeQuantity - quantityMissingRelative);
                                simulatedExtractionMemoryAlternative
                                    .setQuantity(inputPrototype.getPrototype(), quantityMissingTotal);
                                // Original prototype quantity because we what can be extracted from storage and what
                                // could NOT be extracted from storage should be added to the simulation extraction
                                // memory.
                                // The part that could NOT be extracted will be removed later when crafting jobs are
                                // calculated for the missing elements. (not doing so would lead to over-estimation of
                                // what is in storage)
                                simulatedExtractionMemoryBuffer
                                    .add(matcher.withQuantity(inputPrototype.getPrototype(), prototypeQuantity));
                            }
                        } else {
                        ...
                        if (prototypeQuantity == quantityExtracted) {
                            ...
                            missingAlternatives
                                .add(new MissingIngredients.PrototypedWithRequested<>(inputPrototype, quantityMissing));
                        }
                    }
                }
            ...
            if (!hasInputInstance) {
                ...
                if (!collectMissingIngredients) {
                    ...
                    if (missingAlternatives.size() > 0) {
                        missingElements.add(
                            new MissingIngredients.Element<>(
                                missingAlternatives,
                                recipe.isInputReusable(ingredientComponent, inputIndex)));
                    }
                }
            }
        ...
        return Pair.of(inputInstances, collectMissingIngredients ? new MissingIngredients<>(missingElements) : null);
    }
     ...
     */
    @Nullable
    public static IMixedIngredients getRecipeInputs(INetwork network, int channel, IRecipeDefinition recipe,
        boolean simulate, long recipeOutputQuantity) {
        Map<IngredientComponent<?, ?>, List<?>> inputs = getRecipeInputs(
            getNetworkStorageGetter(network, channel, true),
            recipe,
            simulate,
            Maps.newIdentityHashMap(),
            Maps.newIdentityHashMap(),
     ...
     * @param recipe               The recipe to get the inputs from.
     * @param simulate             If true, then the ingredients will effectively be removed from the buffer, not when
     *                             false.
     * @param recipeOutputQuantity The number of times the given recipe should be applied.
     * @return The found ingredients or null.
     */
    ...
    public static IMixedIngredients getRecipeInputsFromCraftingJobBuffer(CraftingJob craftingJob,
        IRecipeDefinition recipe, boolean simulate, long recipeOutputQuantity) {
        Map<IngredientComponent<?, ?>, List<?>> inputs = getRecipeInputs(
            getCraftingJobBufferStorageGetter(craftingJob),
            recipe,
            simulate,
            Maps.newIdentityHashMap(),
            Maps.newIdentityHashMap(),
     ...
     */
    public static Function<IngredientComponent<?, ?>, IIngredientComponentStorage> getNetworkStorageGetter(
        INetwork network, int channel, boolean scheduleObservation) {
     ...
     * @return A callback function for getting a storage for an ingredient component.
     */
    public static Function<IngredientComponent<?, ?>, IIngredientComponentStorage> getCraftingJobBufferStorageGetter(
        CraftingJob craftingJob) {
        return ingredientComponent -> {
            List<?> list = craftingJob.getIngredientsStorageBuffer()
                .getInstances(ingredientComponent);
            return new IngredientComponentStorageSlottedCollectionWrapper<>(
                new IngredientList(ingredientComponent, list),
                Integer.MAX_VALUE,
                Integer.MAX_VALUE);
        };
    }
     ...
     * @param storageGetter               A callback function to get a storage for the given ingredient component.
     * @param recipe                      The recipe to get the inputs from.
     * @param simulate                    If true, then the ingredients will effectively be removed from the network,
     *                                    not when false.
     ...
     */
    public static Pair<Map<IngredientComponent<?, ?>, List<?>>, Map<IngredientComponent<?, ?>, MissingIngredients<?, ?>>> getRecipeInputs(
        Function<IngredientComponent<?, ?>, IIngredientComponentStorage> storageGetter, IRecipeDefinition recipe,
        boolean simulate,
    ...
    public static Pair<Map<IngredientComponent<?, ?>, List<?>>, Map<IngredientComponent<?, ?>, MissingIngredients<?, ?>>> getRecipeInputs(
        ...
        for (IngredientComponent<?, ?> ingredientComponent : recipe.getInputComponents()) {
            ...
            IngredientCollectionPrototypeMap<?, ?> simulatedExtractionMemory = simulatedExtractionMemories
            ...
            Pair<List<?>, MissingIngredients<?, ?>> subIngredients = getIngredientRecipeInputs(
                storage,
            ...
            if (subIngredientAvailable == null && !collectMissingIngredients) {
                ...
                if (collectMissingIngredients && !subIngredientsMissing.getElements()
                    .isEmpty()) {
     ...
     */
    public static <T, M> List<IPrototypedIngredient<T, M>> getCompressedIngredients(
        IngredientComponent<T, M> ingredientComponent, IMixedIngredients mixedIngredients) {
        ...
        for (T instance : mixedIngredients.getInstances(ingredientComponent)) {
            ...
            while (existingIt.hasNext()) {
                ...
                if (matcher
                    .matches(instance, prototypedIngredient.getPrototype(), prototypedIngredient.getCondition())) {
                    T stackedInstance = matcher.withQuantity(
                        prototypedIngredient.getPrototype(),
                        matcher.getQuantity(prototypedIngredient.getPrototype()) + matcher.getQuantity(instance));
                    existingIt.set(
     ...
     */
    public static Map<IngredientComponent<?, ?>, List<IPrototypedIngredient<?, ?>>> getRecipeOutputs(
     ...
     */
    public static <T, M> List<IPrototypedIngredient<T, M>> multiplyPrototypedIngredients(
        Collection<IPrototypedIngredient<T, M>> prototypedIngredients, long amount) {
        return prototypedIngredients.stream()
     ...
     */
    public static <T, M> IPrototypedIngredient<T, M> multiplyPrototypedIngredient(
        IPrototypedIngredient<T, M> prototypedIngredient, long amount) {
    ...
    public static IMixedIngredients mergeMixedIngredients(IMixedIngredients a, IMixedIngredients b) {
        ...
        Map<IngredientComponent<?, ?>, IngredientCollectionQuantitativeGrouper<?, ?, IngredientArrayList<?, ?>>> groupings = Maps
            .newIdentityHashMap();
        for (IngredientComponent<?, ?> component : a.getComponents()) {
            IngredientCollectionQuantitativeGrouper grouping = new IngredientCollectionQuantitativeGrouper<>(
                new IngredientArrayList<>(component));
        ...
        for (Map.Entry<IngredientComponent<?, ?>, IngredientCollectionQuantitativeGrouper<?, ?, IngredientArrayList<?, ?>>> entry : groupings
            .entrySet()) {
    ...
    protected static IMixedIngredients compressMixedIngredients(IMixedIngredients mixedIngredients) {
        ...
        for (Map.Entry<IngredientComponent<?, ?>, IngredientCollectionQuantitativeGrouper<?, ?, IngredientArrayList<?, ?>>> entry : groupings
            .entrySet()) {
            IIngredientMatcher matcher = entry.getKey()
                .getMatcher();
     ...
     * @param storageFallback     The storage to insert any failed ingredients back into. Can be null in simulation
     *                            mode.
    ...
    public static <T, M> boolean insertIngredientCrafting(IngredientComponent<T, M> ingredientComponent,
        ICapabilityProvider capabilityProvider, @Nullable ForgeDirection side, IMixedIngredients ingredients,
        IIngredientComponentStorage<T, M> storageFallback, boolean simulate) {
        IIngredientMatcher<T, M> matcher = ingredientComponent.getMatcher();
        ...
        for (T instance : failedInstances) {
            ...
            if (!matcher.isEmpty(remaining)) {
                throw new IllegalStateException(
                    String.format(
                        "Insertion for a crafting recipe failed "
                            + "due to inconsistent insertion behaviour by destination in simulation "
                            + "and non-simulation: %s. Lost: %s",
                        capabilityProvider,
                        instances));
    ...
    public static boolean insertCrafting(Function<IngredientComponent<?, ?>, PartPos> targetGetter,
        IMixedIngredients ingredients, INetwork network, int channel, boolean simulate) {
        ...
        for (IngredientComponent<?, ?> ingredientComponent : ingredients.getComponents()) {
            TileEntity tile = TileHelpers.get(
                targetGetter.apply(ingredientComponent)
                    .getPos(),
                TileEntity.class)
                .orElse(null);
            if (tile != null) {
        ...
        for (Map.Entry<IngredientComponent<?, ?>, TileEntity> entry : tileMap.entrySet()) {
            IIngredientComponentStorage<?, ?> storageNetwork = simulate ? null
                : getNetworkStorage(network, channel, entry.getKey(), false);
            if (!insertIngredientCrafting(
                (IngredientComponent) entry.getKey(),
                (ICapabilityProvider) entry.getValue(),
                targetGetter.apply(entry.getKey())
                    .getSide(),
                ingredients,
                storageNetwork,
                simulate)) {
     ...
     * @param dependencyGraph     The dependency graph that will be updated if there are dependencies in the original
     *                            job.
    ...
    public static List<CraftingJob> splitCraftingJobs(CraftingJob craftingJob, int splitFactor,
        CraftingJobDependencyGraph dependencyGraph, IIdentifierGenerator identifierGenerator) {
    ...
    public static IMixedIngredients insertIngredients(IMixedIngredients ingredients,
        Function<IngredientComponent<?, ?>, IIngredientComponentStorage> storageGetter, boolean simulate) {
