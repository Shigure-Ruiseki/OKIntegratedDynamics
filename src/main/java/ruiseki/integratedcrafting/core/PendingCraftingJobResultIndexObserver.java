...
public class PendingCraftingJobResultIndexObserver<T, M> implements IIngredientChannelInsertPreConsumer<T> {
    ...
    /**
     * This adds the given instance to the waiting crafting jobs directly, without having to go through the network
     * storage and its observers.
    ...
    public IngredientInstanceWrapper<T, M> addIngredient(IngredientInstanceWrapper<T, M> instanceWrapper, int channel,
        ...
        while (jobsEntryIt.hasNext() && instanceAmount > 0) {
            ...
            if (craftingJob != null && (craftingJob.getChannel() == IPositionedAddonsNetwork.WILDCARD_CHANNEL
                || craftingJob.getChannel() == channel)) {
                    ...
                    if (pendingIngredientsUnsafe != null) {
                        ...
                        while (it.hasNext() && instanceAmount > 0) {
                            ...
                            if (matcher.matches(
                                ...
                                T extracted = matcher
                                    .withQuantity(instanceWrapper.getInstance(), extractedQuantityToAssign);
                                ...
                                IntIterator dependentJobs = craftingJob.getDependentCraftingJobs()
                                    .intIterator();
                                while (dependentJobs.hasNext()) {
                                    CraftingJob dependentJob = craftingNetwork
                                        .getCraftingJob(craftingJob.getChannel(), dependentJobs.nextInt());
                                    if (dependentJob != null) {
                                        long missingQuantity = dependentJob
                                            .getMissingIngredientQuantity(ingredientComponent, extracted);
                                        if (missingQuantity > 0) {
                                            long toExtractQuantity = Math
                                                .min(missingQuantity, extractedQuantityToAssign);
        ...
        return new IngredientInstanceWrapper<>(
            instanceWrapper.getComponent(),
            matcher.withQuantity(instanceWrapper.getInstance(), instanceAmount));
    }
    ...
    protected void onPendingIngredientsEmpty(
        ObjectIterator<Int2ObjectMap.Entry<List<Map<IngredientComponent<?, ?>, List<IPrototypedIngredient<?, ?>>>>>> jobsEntryIt,
        ...
        List<IPrototypedIngredient<T, M>> pendingIngredients, int craftingJobId) {
    ...
    public T insert(int channel, @NotNull T ingredient, boolean simulate) {
        IngredientInstanceWrapper<T, M> instanceWrapper = addIngredient(
            new IngredientInstanceWrapper<>(ingredientComponent, ingredient),
            channel,
            simulate);
