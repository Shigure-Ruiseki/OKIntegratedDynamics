...
public class CraftingJob {
    ...
    public void setLastMissingIngredients(
        Map<IngredientComponent<?, ?>, MissingIngredients<?, ?>> lastMissingIngredients) {
    ...
    public static NBTTagCompound serialize(CraftingJob craftingJob) {
        ...
        tag.setTag(
            "dependencies",
            new NBTTagIntArray(
                craftingJob.getDependencyCraftingJobs()
                    .toIntArray()));
        tag.setTag(
            "dependents",
            new NBTTagIntArray(
    ...
    public static CraftingJob deserialize(NBTTagCompound tag) {
        ...
        if (tag.hasKey("lastMissingIngredients", Constants.NBT.TAG_LIST)) {
            Map<IngredientComponent<?, ?>, MissingIngredients<?, ?>> lastMissingIngredients = MissingIngredients
                .deserialize(tag.getTagList("lastMissingIngredients", Constants.NBT.TAG_COMPOUND));
            craftingJob.setLastMissingIngredients(lastMissingIngredients);
        } else {
            craftingJob.setLastMissingIngredients(Maps.newIdentityHashMap());
        }
    ...
    public String toString() {
        return String.format(
            "[Crafting Job id: %s, channel: %s, recipe: %s, dependencies: %s, dependents: %s, amount: %s, storage: %s]",
            getId(),
            getChannel(),
            getRecipe(),
            getDependencyCraftingJobs(),
            getDependentCraftingJobs(),
            getAmount(),
