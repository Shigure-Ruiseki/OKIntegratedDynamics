...
public class UnknownCraftingRecipeException extends Exception {
    ...
    public UnknownCraftingRecipeException(IPrototypedIngredient<?, ?> ingredient, long quantityMissing,
        List<UnknownCraftingRecipeException> missingChildRecipes, IMixedIngredients ingredientsStorage,
    ...
    public String getMessage() {
        return String.format(
            "Could not find a recipe for %s (with %s missing, and %s stored, and %s partial), with missing sub-recipes: %s",
            getIngredient(),
            getQuantityMissing(),
            getIngredientsStorage(),
            getPartialCraftingJobs(),
            getMissingChildRecipes());
    }
    ...
    public boolean equals(Object obj) {
        ...
        return this.getIngredient()
            .equals(that.getIngredient()) && this.getQuantityMissing() == that.getQuantityMissing()
            && this.getMissingChildRecipes()
                .equals(that.getMissingChildRecipes())
            && this.getIngredientsStorage()
                .equals(that.getIngredientsStorage())
            && this.getPartialCraftingJobs()
                .equals(that.getPartialCraftingJobs());
    }
}
