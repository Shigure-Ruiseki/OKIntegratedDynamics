...
public class HandlerWrappedTerminalCraftingOption<T> {
    ...
    public static <T> NBTTagCompound serialize(HandlerWrappedTerminalCraftingOption<T> craftingOption) {
        ...
        tag.setString(
            "craftingOptionHandler",
            handler.getId()
                .toString());
        return tag;
    }

    public static <T, M> HandlerWrappedTerminalCraftingOption<T> deserialize(
        IngredientComponent<T, M> ingredientComponent, NBTTagCompound tag) {
