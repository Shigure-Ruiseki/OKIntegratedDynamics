...
public class PendingCraftingJobOutputs<T, M> {
     ...
     */
    public static <T, M> PendingCraftingJobOutputs<T, M> collectFromNetwork(
        IngredientComponent<T, M> ingredientComponent, INetwork network, int channel) {
    ...
    public void add(T instance, TerminalCraftingJobStatus status) {
        ...
        if (existingOutput != null) {
            instance = matcher.withQuantity(
                instance,
                addQuantities(
                    matcher,
                    matcher.getQuantity(existingOutput.getInstance()),
                    matcher.getQuantity(instance)));
            if (existingOutput.getStatus()
                .getPriority() >= status.getPriority()) {
    ...
    public void addCraftingPlan(ITerminalCraftingPlan<?> craftingPlan, Set<Object> handledPlans) {
        // Jobs can occur multiple times within a plan due to job splitting, so only take each of them into account
        // once.
        ...
        // so this status does not mean that the job is done: a job that is done is not exposed as a running job
        // anymore.
        ...
        TerminalCraftingJobStatus status = craftingPlan.getStatus() == TerminalCraftingJobStatus.FINISHED
            ? TerminalCraftingJobStatus.CRAFTING
