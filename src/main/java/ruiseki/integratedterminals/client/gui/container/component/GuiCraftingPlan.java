...
public class GuiCraftingPlan extends Gui {
    ...
    public void drawGuiContainerLayer(int guiLeft, int guiTop, GuiTerminalStorage.DrawLayer layer, float partialTick,
        ...
        for (GuiCraftingPlan.Element element : getVisibleElements()) {
            drawElement(
                element,
                getAbsoluteElementIndent(element),
                guiLeft + x,
                guiTop + y + offsetY,
                ELEMENT_WIDTH,
                ELEMENT_HEIGHT,
    ...
    private void drawElement(Element element, int indent, int x, int y, int width, int height,
        ...
        if (!element.getChildren()
            .isEmpty() && layer == GuiTerminalStorage.DrawLayer.BACKGROUND) {
            GlStateManager.color(1, 1, 1, 1);
            Image image = element.getChildren()
                .get(0)
                .isEnabled() ? Images.ARROW_DOWN : Images.ARROW_RIGHT;
            image.draw(this, x, y);
        }
        x += 16;
        ...
        if (layer == GuiTerminalStorage.DrawLayer.BACKGROUND) {
            ...
            if (element.getCraftQuantity() > 0) {
                ...
                RenderHelpers.drawScaledString(
                    Minecraft.getMinecraft().fontRenderer,
                    LangHelpers
                        .localize("gui.integratedterminals.terminal_storage.crafting", element.getCraftQuantity()),
                    x + 9,
                    y + 1,
                    0.5F,
                    16777215,
            ...
            GuiHelpers.renderTooltipOptional(this.parentGui, x, y, 50, GuiHelpers.SLOT_SIZE, mouseX, mouseY, () -> {
                String unlocalizedName = "gui.integratedterminals.craftingplan.status." + element.getStatus()
                    .name()
                    .toLowerCase(Locale.ENGLISH) + ".";
                return Optional.of(
                    Lists.newArrayList(
                        LangHelpers.localize(unlocalizedName + "name"),
                        LangHelpers.localize(unlocalizedName + "desc")));
            });
        }
    }
    ...
    protected static void addElements(@Nullable Element parent, int indent, ITerminalCraftingPlan<?> craftingPlan,
        List<GuiCraftingPlan.Element> elements) {
        boolean valid = craftingPlan.getStatus()
            .isValid()
            || (!craftingPlan.getStorageIngredients()
                .isEmpty()
                ...
                elements.add(
                    currentElement.addChild(
                        new Element(
                            ...
                            0,
                            craftingPlan.getCraftingQuantity(),
                            TerminalCraftingJobStatus.INVALID.getColor(),
                    ...
                    currentElement.addChild(
                        new Element(
                            ...
                            storageIngredient.getComponent()
                                .getMatcher()
                                .getQuantity(storageIngredient.getPrototype()),
                            0,
                            0,
                            TerminalCraftingJobStatus.FINISHED.getColor(),
                            TerminalCraftingJobStatus.FINISHED)));
            }
        }
