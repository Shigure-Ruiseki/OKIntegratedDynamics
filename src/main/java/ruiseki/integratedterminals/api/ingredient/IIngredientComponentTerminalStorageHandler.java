...
public interface IIngredientComponentTerminalStorageHandler<T, M> {
    ...
    public void drawInstance(T instance, long maxQuantity, @Nullable String label, GuiContainer gui,
    ...
    public default void addQuantityTooltip(List<String> lines, T instance) {
        String line = EnumChatFormatting.DARK_GRAY + LangHelpers
            .localize("gui.integratedterminals.terminal_storage.tooltip.quantity", formatQuantity(instance));
        if (lines.size() <= 1) {
            lines.add(line);
        } else {
            lines.add(1, line);
    ...
    /**
     * @return The number that should be removed when right-clicking when an instance is selected in the storage
     *         terminal.
     */
    ...
    public default void insertMaxIntoContainer(IIngredientComponentStorage<T, M> storage, Container container,
        ...
        IIngredientMatcher<T, M> matcher = storage.getComponent()
            .getMatcher();
        T toAdd = instance;
        ...
        while (!matcher.isEmpty(toAdd) && slot < containerSlotEnd) {
            ...
            if (!matcher.isEmpty(moved)) {
                toAdd = matcher
                    .withQuantity(toAdd, Math.max(0, matcher.getQuantity(toAdd) - matcher.getQuantity(moved)));
            }
        }
    }
     ...
     * @param player                The player. If null, the container slot will not be picked up by the player if not
     *                              empty.
     * @param transferFullSelection If the selected stack should be moved fully.
     * @return The instance quantity that was moved.
     */
    public T insertIntoContainer(IIngredientComponentStorage<T, M> storage, Container container, int containerSlot,
        T maxInstance, @Nullable EntityPlayer player, boolean transferFullSelection);
     ...
     */
    public void extractActiveStackFromPlayerInventory(IIngredientComponentStorage<T, M> storage,
     ...
     */
    public void extractMaxFromContainerSlot(IIngredientComponentStorage<T, M> storage, Container container,
