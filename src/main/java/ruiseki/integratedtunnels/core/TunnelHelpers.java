...
public class TunnelHelpers {
    ...
    public static <T, M> T moveSingle(IIngredientComponentStorage<T, M> source, int sourceSlot,
        ...
        IngredientPredicate<T, M> ingredientPredicate, PartPos movementPosition, boolean simulate)
            ...
            try {
                if (ingredientPredicate.hasMatchFlags()) {
                    ...
                    return IngredientStorageHelpers.moveIngredientsSlotted(
                        ...
                        destinationSlot,
                        ingredientPredicate,
                        ingredientPredicate.getMaxQuantity(),
                        ingredientPredicate.isExactQuantity(),
                        simulate);
                }
                ...
                throw new EvaluationException(
                    "Ingredient movement failed "
                        ...
                        + e.getIngredientComponent()
                            .getMatcher()
                            .localize(e.getRemainder())
                        + ".");
            }
     ...
     */
    @Nonnull
    public static <T, M> T moveSingleStateOptimized(INetwork network,
        IPositionedAddonsNetworkIngredients<T, M> ingredientsNetwork, int channel, ITunnelConnection connection,
        IIngredientComponentStorage<T, M> source, int sourceSlot, IIngredientComponentStorage<T, M> destination,
        int destinationSlot, IngredientPredicate<T, M> ingredientPredicate, PartPos movementPosition,
        boolean craftIfFailed) throws EvaluationException {
        IIngredientMatcher<T, M> matcher = source.getComponent()
            .getMatcher();
     ...
     */
    public static <T, M> boolean requestCrafting(INetwork network,
        IPositionedAddonsNetworkIngredients<T, M> ingredientsNetwork, int channel, T instance, M matchCondition) {
     ...
     */
    public static <T, M> boolean isCrafting(INetwork network,
        IPositionedAddonsNetworkIngredients<T, M> ingredientsNetwork, int channel, T instance, M matchCondition) {
        return IntegratedDynamics._instance.getRegistryManager()
            .getRegistry(INetworkCraftingHandlerRegistry.class)
            .isCrafting(
