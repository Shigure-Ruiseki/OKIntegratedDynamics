...
import ruiseki.integratedtunnels.core.predicate.IngredientPredicateFluidStackList;
import ruiseki.integratedtunnels.core.predicate.IngredientPredicateFluidStackNbt;
...
public class TunnelFluidHelpers {
    ...
    public static IngredientPredicate<FluidStack, Integer> matchAll(final int amount, final boolean exactAmount) {
        return new IngredientPredicate<FluidStack, Integer>(
            IngredientComponent.FLUIDSTACK,
            new FluidStack(FluidRegistry.WATER, amount),
            exactAmount ? FluidMatch.AMOUNT : FluidMatch.ANY,
            false,
            false,
            amount,
            exactAmount) {
    ...
    public static IngredientPredicate<FluidStack, Integer> matchFluidStacks(
        final IValueTypeListProxy<ValueObjectTypeFluidStack, ValueObjectTypeFluidStack.ValueFluidStack> fluidStacks,
        final boolean checkFluid, final boolean checkAmount, final boolean checkNbt, final boolean blacklist,
        final int amount, final boolean exactAmount) {
        return new IngredientPredicateFluidStackList(
            blacklist,
            amount,
            exactAmount,
            fluidStacks,
    ...
    public static IngredientPredicate<FluidStack, Integer> matchPredicate(final PartTarget partTarget,
    ...
    public static boolean areFluidStackEqual(FluidStack stackA, FluidStack stackB, boolean checkFluid,
        boolean checkAmount, boolean checkNbt) {
     ...
     */
    public static FluidStack placeFluids(INetwork network,
        IPositionedAddonsNetworkIngredients<FluidStack, Integer> ingredientsNetwork, int channel,
        ITunnelConnection connection, IIngredientComponentStorage<FluidStack, Integer> source, final World world,
        final BlockPos pos, IngredientPredicate<FluidStack, Integer> fluidStackMatcher, boolean blockUpdate,
        boolean ignoreReplacable, boolean craftIfFailed) throws EvaluationException {
        Block destBlockState = pos.getBlock(world);
        final Material destMaterial = destBlockState.getMaterial();
        final boolean isDestNonSolid = !destMaterial.isSolid();
        final boolean isDestReplaceable = destBlockState.isReplaceable(world, pos.getX(), pos.getY(), pos.getZ());
        if (!world.isAirBlock(pos.getX(), pos.getY(), pos.getZ())
            && (!isDestNonSolid || !(ignoreReplacable && isDestReplaceable) || destMaterial.isLiquid())) {
            return null;
        }

        IIngredientComponentStorage<FluidStack, Integer> destination = new FluidStorageBlockWrapper(
            (WorldServer) world,
            pos,
            null,
            blockUpdate);
     ...
     * @throws EvaluationException If illegal movement occured and further movement should stop.
     */
    public static FluidStack pickUpFluids(INetwork network,
        IPositionedAddonsNetworkIngredients<FluidStack, Integer> ingredientsNetwork, int channel,
        ...
        if (block instanceof IFluidBlock || block instanceof BlockLiquid) {
            IIngredientComponentStorage<FluidStack, Integer> source = new FluidStorageBlockWrapper(
                (WorldServer) world,
                pos,
                side,
                false);
