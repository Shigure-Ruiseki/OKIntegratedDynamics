 ...
 * @author rubensworks
 */
public class PartTypePlayerSimulator
    extends PartTypeTunnelAspectsWorld<PartTypePlayerSimulator, PartStatePlayerSimulator> {
    ...
    protected PartStatePlayerSimulator constructDefaultState() {
        return new PartStatePlayerSimulator(
    ...
    public int getConsumptionRate(PartStatePlayerSimulator state) {
        return state.hasVariable() ? GeneralConfig.playerSimulatorBaseConsumptionEnabled
            : GeneralConfig.playerSimulatorBaseConsumptionDisabled;
    }
